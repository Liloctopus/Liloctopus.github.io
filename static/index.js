(function(w) {
    ...
    w.shareText = function(score) {
        ...
        SubmitResults();
        if (score <= 49) return '继续加油噢';
        if (score <= 99) return '然然爱你噢';
        if (score <= 149) return 'tql然然sl';
    ...
    w.initSetting = function() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "吃掉小然然";
        if (cookie("keyboard")) {
